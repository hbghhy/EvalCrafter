...
def get_quality_score(aesthetic_score, technical_score):
    quality_weights = np.array([0.03004555, 0.02887537])*5
    quality_intercept = 0.08707462696457707*5
    quality = aesthetic_score * quality_weights[0]/ 100 + technical_score * quality_weights[1]/ 100 + quality_intercept
    return quality * 100
...
if __name__ == "__main__":
    ...
    for i, data in enumerate(tqdm(dataloader, desc="Testing")):
        ...
        file_name = os.path.basename(data["name"][0])
        video_gen_id = os.path.splitext(file_name)[0]  # Extract video_gen_id (file name without extension)
        ...
        results_list.append({
            ...
            'quality_score': get_quality_score(rescaled_results["aesthetic"] * 100, rescaled_results["technical"] * 100),
        })
