...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(video_paths))):
        ...
        video_gen_id = os.path.splitext(file_name)[0] 
        prompt_row = prompts_df[prompts_df['video_gen_id'] == video_gen_id]

        
        if prompt_row.empty:
            logging.warning(f"No prompt found for video: {video_gen_id}")
            continue
        
        prompt_text = prompt_row['prompt'].values[0]
        if count == 0:
